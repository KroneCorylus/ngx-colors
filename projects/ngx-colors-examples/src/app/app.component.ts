      ...
      transition('* <=> *', [
        query(
          ...
          [
            ...
            stagger(40, [animate('180ms ease-out', style({ opacity: 1, transform: 'translateY(0)' }))]),
          ],
          { optional: true },
        ),
      ]),
...
export class AppComponent implements OnDestroy {
  constructor(
    ...
    this.playgroundForm.valueChanges.pipe(takeUntil(this.destroy$)).subscribe((raw) => {
      if (!raw) return;
  ...
  readonly layoutOptions: LayoutOptions[] = ['pages', 'full-vertical', 'full-horizontal'];
  ...
  readonly paletteChoices: PaletteChoice[] = [
    ...
    },
    {
      ...
      description: 'Demonstrates passing an observable palette with delayed data.',
      palette$: of([
  ...
  readonly presets: PlaygroundPreset[] = [
    {
      ...
      description: 'Compact picker that only shows the swatch palette and auto-applies on click.',
      formValue: {
        layout: 'pages',
        displayText: false,
    ...
    },
    {
      ...
      description: 'Locked hue with alpha slider, great for monochromatic themes.',
      formValue: {
        layout: 'full-vertical',
        displayText: true,
        displaySliders: true,
    ...
    },
    {
      ...
      description: 'Requires confirm on every input type and keeps overlay attached to the log panel.',
      formValue: {
    ...
    },
    {
      ...
      description: 'Disables palette, limits color models, and customises action labels.',
      formValue: {
  ...
  readonly playgroundControl = new FormControl<string | null>('rgba(0, 153, 255, 0.8)');
  ...
  readonly playgroundForm = this.fb.group({
    ...
    outputModel: this.fb.control<ColorModel | 'AUTO'>('AUTO', { nonNullable: true }),
    allowedModels: this.fb.control<ColorModel[]>([...this.colorModels], { nonNullable: true }),
    eyedropper: this.fb.control(true, { nonNullable: true }),
    animation: this.fb.control<AnimationOptions>('popup', { nonNullable: true }),
    overlayClass: this.fb.control<string>('', { nonNullable: true }),
    overlayAttachTo: this.fb.control<'default' | 'playground' | 'log'>('default', {
      nonNullable: true,
    }),
  ...
  private getPaletteChoice(id: string): PaletteChoice {
    return this.paletteChoices.find((option) => option.id === id) ?? this.paletteChoices[0];
  }
  ...
  presetConfig(preset: PlaygroundPreset): NgxColorsConfiguration {
    const merged = this.normalizeFormValue(preset.formValue, this.defaultFormValue);
  ...
  buildConfig(
    ...
    const resolvedChoice = paletteChoice ?? this.getPaletteChoice(formValue.paletteSource);
    ...
    if (formValue.lockSaturationEnabled) lockValues.saturation = Number(formValue.lockSaturation);
    if (formValue.lockBrightnessEnabled) lockValues.brightness = Number(formValue.lockBrightness);
    if (formValue.lockAlphaEnabled) lockValues.alpha = Number(formValue.lockAlpha);

    const confirmationActive =
      formValue.confirmationPalette || formValue.confirmationText || formValue.confirmationSliders;

    const confirmation: ConfirmationRequiredOptions | undefined = confirmationActive
      ? {
    ...
    return {
      ...
      overlayClass,
      overlayAttachTo,
      palette: resolvedChoice.palette$ ?? resolvedChoice.palette ?? defaultColors,
  ...
  toggleAllowedModel(model: ColorModel, checked: boolean) {
    ...
    if (current.length === 1) {
      this.logEvent('Playground', 'allowedModels', 'At least one color model is required.');
      return;
    }
    this.allowedModelsControl.setValue(current.filter((item) => item !== model));
  }
